...
kordat[9:ncol(kordat)]<- lapply(kordat[9:ncol(kordat)], function(x) factor(x))
sink("results.txt")
...
std <- tapply(kordat$Average ,kordat$f, sd, na.rm=TRUE)
cat("Standartnovirze:\n")
print(std)
...
dev.off()
svg("boxplot.svg")
boxplot(kordat$Intercept ~ kordat$f, col = rainbow(length(unique(kordat$f))), outpch = 16,
