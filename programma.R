kordat <- read.table(file="variants20.txt", header=TRUE, dec=",", row.names=1, strip.white=TRUE, sep="\t", stringsAsFactors = TRUE)
...
print(sl.by.b)
kordat$Average <- apply(kordat[,c("Slope", "Intercept", "adj.r.squared")], 1, mean, na.rm=TRUE)
# Piemērs ar secīgu tapply un lapply
std_all <- lapply(c("Slope", "Intercept", "adj.r.squared", "Average"), function(col) {
  tapply(kordat[[col]], kordat$f, sd, na.rm = TRUE)
})
cat("Standartnovirzes pa f faktora līmeņiem:\n")
print(std_all)
prockordat <- kordat[kordat$adj.r.squared > 0.7, ]
prockordat$Slope <- (function(x) { 1 - 1 / x })(prockordat$Slope)
